...
class RentalServiceTest {
    ...
    fun bookingTotalIsCalculatedServerSideAndWalletIsDebited() {
        ...
        when(cars.findById(7L)).thenReturn(Optional.of(car))
        when(bookings.existsOverlapping(Mockito.eq(7L), any(), any(), any())).thenReturn(false)
        doNothing().when(wallet).reserve(42L, BigDecimal("6000.00"))
        when(wallet.finalizeReservedDebit(Mockito.eq(42L), Mockito.eq(BigDecimal("6000.00")), any(), any()))
        ...
        verify(wallet, times(1)).finalizeReservedDebit(Mockito.eq(42L), Mockito.eq(BigDecimal("6000.00")), any(), any())
    }
}
