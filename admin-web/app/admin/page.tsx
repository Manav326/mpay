...
function UsersView({users,role,visibleRoles,roleFilter,setRoleFilter,sort,setSort,selected,setSelected,canManageUserStatus,onStatusChanged}:{users:UserSummary[];role:Role;visibleRoles:string[];roleFilter:Role|'ALL';setRoleFilter:(v:any)=>void;sort:SortMode;setSort:(v:any)=>void;selected?:UserDetail;setSelected:(v:any)=>void;canManageUserStatus:boolean;onStatusChanged:(id:string,status:'ACTIVE'|'BLOCKED')=>void}){
...
function UserDrawer({user,onClose,canManageUserStatus,onStatusChanged}:{user:UserDetail;onClose:()=>void;canManageUserStatus:boolean;onStatusChanged:(id:string,status:'ACTIVE'|'BLOCKED')=>void}){
  ...
  return <div className="drawer-overlay" onClick={onClose}>
    <aside className="user-drawer user-drawer-wide" onClick={e=>e.stopPropagation()}>
      ...
      {tab==='wallet' && <section className="drawer-section">
       ...
       {canManageUserStatus && user.role.toUpperCase() !== 'ADMIN' && <div className="drawer-note user-status-control"><div><b>Account access</b><span>Use this only for support or operational access control.</span>{statusError&&<em>{statusError}</em>}</div><button className={user.status==='ACTIVE'?'text-danger-btn':'primary'} disabled={statusBusy} onClick={async()=>{setStatusBusy(true);setStatusError('');try{const next=await updateUserStatus(user.id,user.status!=='ACTIVE');onStatusChanged(user.id,next.status); }catch(err:any){setStatusError(err.message||'Unable to update account status.')}finally{setStatusBusy(false)}}}>{statusBusy?'Saving…':user.status==='ACTIVE'?'Block account':'Activate account'}</button></div>}
