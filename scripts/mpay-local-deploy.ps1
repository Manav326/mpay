...
Write-Host ""
Write-Host "mPay local Azure deployment rehearsal" -ForegroundColor Cyan
Write-Host "Branch         : $Branch"
Write-Host "Deploy checkout: $DeployDirectory"
Write-Host ""
...
if (-not (Test-Path (Join-Path $DeployDirectory ".git"))) {
    Write-Host "Creating a dedicated deployment checkout..." -ForegroundColor Yellow
...
if (Test-Path $sourceConfig) {
    ...
    Write-Host "Copied local backend/config into deployment checkout." -ForegroundColor Yellow
} elseif (-not (Test-Path $targetConfig)) {
    throw "backend/config is missing. Provide the existing local backend/config before deployment."
}

foreach ($port in @(3000, 8080)) {
    $listener = Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue
    if ($listener) { throw "Port $port is already in use. Stop the existing mPay stack first; this script will not stop or alter it." }
}

$env:MPAY_BACKEND_IMAGE = "ghcr.io/manav326/mpay-backend"
$env:MPAY_ADMIN_WEB_IMAGE = "ghcr.io/manav326/mpay-admin-web"
$env:MPAY_IMAGE_TAG = "local-sha-$commitSha"
...
Write-Host "Image tag        : $($env:MPAY_IMAGE_TAG)" -ForegroundColor Green

Write-Host "Building the exact deployment checkout..." -ForegroundColor Yellow
Invoke-Compose ($composeFiles + @("-p", $project, "build", "backend", "admin-web"))

Write-Host "Starting isolated PostgreSQL and Redis..." -ForegroundColor Yellow
...
Write-Host "Committed deployment SHA: $commitSha" -ForegroundColor Cyan
